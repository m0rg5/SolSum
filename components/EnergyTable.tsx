...
interface EnergyTableProps {
  ...
  onContextClick: (item: PowerItem) => void;
...
const isMgmt = (item: PowerItem) => 
  ...
  item.category === LoadCategory.SYSTEM_OVERHEAD;
...
const EnergyTable: React.FC<EnergyTableProps> = ({ items, systemVoltage, onUpdateItem, onDeleteItem, onAddItem, onAIAddItem, onContextClick, onReorder, onSort }) => {
    ...
    return (
      <React.Fragment key={category}>
        <tr className="bg-slate-950/80 border-t-2 border-slate-800">
          <td colSpan={10} className="px-4 py-3"><span className="app-header-font text-[10px] text-blue-400">{category}</span></td>
        </tr>
            ...
            <tr key={item.id} className={`border-b border-slate-800 hover:bg-slate-800/40 transition-colors group ${draggedId === item.id ? 'opacity-50 bg-slate-800' : ''} ${managementItem ? 'bg-slate-900/40 opacity-60' : ''}`} draggable
              ...
              <td className="px-4 py-3 text-right font-mono text-amber-400 font-bold text-sm whitespace-nowrap relative">
                <div className="flex items-center justify-end gap-2">{(ah || 0).toFixed(1)}
                   {isSuspicious && <button onClick={() => onContextClick(item)} className="text-amber-500 animate-pulse"><svg viewBox="0 0 20 20" fill="currentColor" className="w-4 h-4"><path fillRule="evenodd" d="M8.485 2.495c.673-1.167 2.357-1.167 3.03 0l6.28 10.875c.673 1.167-.17 2.625-1.516 2.625H3.72c-1.347 0-2.189-1.458-1.515-2.625L8.485 2.495ZM10 5a.75.75 0 0 1 .75.75v3.5a.75.75 0 0 1-1.5 0v-3.5A.75.75 0 0 1 10 5Zm0 9a1 1 0 1 0 0-2 1 1 0 0 0 0 2Z" clipRule="evenodd" /></svg></button>}
                </div>
              </td>
              ...
              <td className="px-2 py-3 text-center w-10"><button onClick={() => onContextClick(item)} className="w-7 h-7 flex items-center justify-center rounded-lg bg-slate-800 hover:bg-blue-600 text-slate-400 border border-slate-700 text-xs font-bold shadow-sm">➕</button></td>
