...
import { SystemTotals, PowerItem, ChargingSource, BatteryState } from '@/types';
...
const SummaryPanel: React.FC<SummaryPanelProps> = ({ totals, systemVoltage, items, battery, charging }) => {
    ...
    const renderAutonomyRow = (label: string, scenario: 'peak' | 'cloud' | 'current' | 'zero', icon: React.ReactNode) => {
        ...
        return (
            <div className="flex items-start gap-3 w-full group/row py-0.5" key={label}>
    ...
    return (
        <div className="flex flex-col items-center gap-4 w-full text-center py-1 px-1">
            ...
            <div className="w-full bg-slate-950 p-4 rounded-xl border border-slate-800 shadow-2xl flex flex-col items-center relative overflow-hidden">
                ...
                <div className="w-full space-y-2.5 px-1 max-w-[170px]">
                    ...
                    {renderAutonomyRow("Reserve", "zero", "🔋")}
                </div>
            </div>
        </div>
    );
