...
export const calculateSystemTotals = (
  ...
  charging.forEach(source => {
    ...
    if (source.autoSolar && source.type === 'solar' && battery.forecast && !battery.forecast.loading) {
       hours = Number(battery.forecast.sunnyHours) || 0; 
    }
...
export const calculateAutonomy = (
  ...
  if (scenario !== 'zero') {
    charging.forEach(source => {
      ...
      if (scenario === 'current' && source.autoSolar && source.type === 'solar' && solarForecast) {
        h = Number(solarForecast.sunny) || 0;
      }
      if (source.type === 'solar') {
        if (scenario === 'peak' && solarForecast) h = Number(solarForecast.sunny) || 0;
        if (scenario === 'cloud' && solarForecast) h = Number(solarForecast.cloudy) || 0;
      }
  ...
  // Invariant: Generation >= Consumption means infinity
  if (netWhPerDay >= 0) {
  ...
  return { 
    days: isFinite(days) ? days : Infinity, 
    hours: isFinite(days) ? days * 24 : Infinity, 
    netWh: netWhPerDay 
  };
};
