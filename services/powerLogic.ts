...
export const getInverterEfficiency = (watts) => {
 ...
 */
export const normalizeAutoSolarHours = (battery) => {
...
export const calculateItemEnergy = (item, systemVoltage) => {
...
export const calculateSystemTotals = (items, charging, battery) => {
...
export const calculateAutonomy = (items, charging, battery, scenario, solarForecast, currentSoC) => {
    const systemVoltage = Number(battery.voltage) || 24;
    let dailyWhConsumed = 0;
    items.forEach(item => {
        if (item.enabled === false)
        ...
        charging.forEach(source => {
            ...
            if (source.type === 'solar') {
                ...
                if (scenario === 'peak') {
                     ...
                     * Priority 1: Use the Historical Monthly Average (Sunny PSH) for this location/month.
                     * Priority 2: Fallback to 70% of peak if no location data exists (Generic Baseline).
                     * 
                     * This row now answers: "Is my system sustainable across the average month in this location?"
                     */
                    const historicalAverage = (source.autoSolar && solarForecast?.sunny) ? solarForecast.sunny : 0;

                    if (historicalAverage > 0) {
                        h = historicalAverage;
                    } else {
                        // Fallback: 70% of sunny hours for a standard "seasonal baseline" when location is unknown.
                        h = sunnyHours * 0.70;
                    }
                }
    ...
    /**
     * Basis Logic: 
     * All autonomy scenarios should use the CURRENT SoC to provide comparable projections.
     */
